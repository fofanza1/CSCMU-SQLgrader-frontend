...
export class SubmissionComponent implements OnInit {
  ...
  DBMs = [
    {value: "pg", name: "Postgres"}, {value: "mysql", name: "MySQL, MariaDB"},
  ...
  constructor(
      ...
      private taskSerivce: TasksService, public snackBar: MatSnackBar) {
    this.sqlFile = new FileUploader({url: this.URL, itemAlias: "submitfile"});
  ...
  openSnackBar(message: string) {
    this.snackBar.open(
        message, null, {duration: 3000, horizontalPosition: "center"});
  }
    ...
    this.route.params.subscribe(async params => {
      ...
      this.assignmentService
          .getAssignemntsDetailByNumberInUser(
              localStorage.getItem("courseId"), this.assignmentNumber$)
          .subscribe(
              data => {
                console.log(data);
                ...
                if (this.dataAssignement$.astatus === "opening" &&
                    moment(this.dataAssignement$.duedate)
              ...
              err => { this.router.navigate(["/home/assignment"]); });
    });
  }
  ...
  downloadAssignmentFile() {
    this.assignmentService.downloadassignment(this.dataAssignement$.aid)
        .subscribe(res => {
  ...
  downloadScriptFile() {
    this.assignmentService.downloadScriptAssignment(this.dataAssignement$.aid)
        .subscribe(res => {
  ...
  subMissionDetail() {
    this.taskSerivce.submissionDetail(this.dataAssignement$.aid)
        .subscribe(res => {
  ...
  goToAssignments() { this.router.navigate(["/home/assignment"]); }

  detectFile() {
    var name =
        this.sqlFile.queue[0].file.name.replace(".sql", "").toLowerCase();
